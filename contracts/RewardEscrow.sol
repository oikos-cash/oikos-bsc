...
// https://docs.oikos.cash/contracts/RewardEscrow
contract RewardEscrow is Owned, IRewardEscrow {
    ...
    function setOikos(IOikos _oikos) external onlyOwner {
        oikos = _oikos;
        emit OikosUpdated(address(_oikos));
    }
    ...
    function _appendVestingEntry(address account, uint quantity) internal {
        ...
        require(
            totalEscrowedBalance <= IERC20(address(oikos)).balanceOf(address(this)),
    ...
    function vest() external {
        ...
        if (total != 0) {
            ...
            IERC20(address(oikos)).transfer(msg.sender, total);
