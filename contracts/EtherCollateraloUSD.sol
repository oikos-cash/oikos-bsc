...
// ETH Collateral v0.3 (oUSD)
// https://docs.synthetix.io/contracts/source/contracts/ethercollateralsusd
contract EtherCollateraloUSD is Owned, Pausable, ReentrancyGuard, MixinResolver, IEtherCollateraloUSD {
    ...
    constructor(address _owner, address _resolver) public Owned(_owner) Pausable() MixinResolver(_resolver, addressesToCache) {
        vToken = address(0xA07c5b74C9B40447a954e1466938b865b6BBea36);
        ...
        returns (uint256 loanID)
    {
        ...
        uint scaledAmount = _amount * (10**(18-8)) ;
    ...
    function depositCollateral(address account, uint256 loanID, uint256 _amount) external payable notPaused {
    ...
    function withdrawCollateral(uint256 loanID, uint256 withdrawAmount) external notPaused nonReentrant ETHRateNotInvalid {
        ...
        uint256 scaledAmount = withdrawAmount / (10**(18-8)) ;
        
        // transfer ETH to msg.sender
        IERC20(vToken).transfer(msg.sender, scaledAmount);
    ...
    function liquidateLoan(
        ...
        uint256 collateralRedeemed = exchangeRates().effectiveValue(oUSD, amountToLiquidate, COLLATERAL);
    ...
    function _processInterestAndLoanPayment(uint256 interestPaid, uint256 loanAmountPaid) internal {
        // Fee distribution. Mint the oUSD fees into the FeePool and record fees paid
        if (interestPaid > 0) {
            synthoUSD().issue(FEE_ADDRESS, interestPaid);
    ...
    function _closeLoan(
        ...
        IERC20(vToken).transfer(synthLoan.account, remainingCollateral);
   ...
   function synthoBNB() internal view returns (ISynth) {
        return ISynth(resolver.requireAndGetAddress(CONTRACT_SYNTHOBNB, "Missing SynthoBNB address"));
    }
