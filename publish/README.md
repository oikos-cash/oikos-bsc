...
- `-n, --network <value>` The network to run off. One of bsc, testnet, rinkeby, rospen. (default: "testnet")
...
node publish deploy -n testnet -d publish/deployed/testnet -g 8
...
node publish verify -n testnet -d publish/deployed/testnet
```
...
Will attempt purge the given synth with all token holders it can find. Uses the list of holders from bsc, and as such won't do anything for other networks.
...
node publish generate-token-list -d publish/deployed/bsc
