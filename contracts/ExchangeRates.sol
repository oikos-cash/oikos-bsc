...
// https://docs.oikos.cash/contracts/source/contracts/ExchangeRates
contract ExchangeRates is Owned, SelfDestructible, IExchangeRates {
    ...
    // Exchange rates and update times stored by currency code, e.g. 'OKS', or 'oUSD'
    ...
    function ratesAndStaleForCurrencies(bytes32[] calldata currencyKeys) external view returns (uint[] memory, bool) {
        ...
        for (uint i = 0; i < currencyKeys.length; i++) {
            ...
            if (!anyRateStale) {
                anyRateStale = (currencyKeys[i] != "oUSD" && uint256(rateAndUpdateTime.time).add(period) < now);
            }
        }
    ...
    function rateIsStale(bytes32 currencyKey) external view returns (bool) {
        // oUSD is a special case and is never stale.
    ...
    function anyRateIsStale(bytes32[] calldata currencyKeys) external view returns (bool) {
        ...
        while (i < currencyKeys.length) {
            // oUSD is a special case and is never false
            if (currencyKeys[i] != "oUSD" && _getUpdatedTime(currencyKeys[i]).add(rateStalePeriod) < now) {
                return true;
            }
            i += 1;
        }
