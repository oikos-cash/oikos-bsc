...
contract('ExchangeState', accounts => {
	...
	const [oUSD, oBTC, sAUD] = ['oUSD', 'oBTC', 'sAUD'].map(toBytes32);
	...
	describe('adding, removing, selecting and length of entries', () => {
		it('the length is 0 by default', async () => {
			const length = await exchangeState.getLengthOfEntries(account1, oBTC);
			assert.equal(length, '0');
		});
		it('only the associated contract can invoke appendExchangeEntry()', async () => {
			await onlyGivenAddressCanInvoke({
				fnc: exchangeState.appendExchangeEntry,
				args: [account1, oUSD, toUnit('1'), oBTC, toUnit('1'), toUnit('0.01'), '0', '0', '0'],
		...
		describe('when an entry is added to oBTC for the first user', () => {
			...
			it('then the length is 1 for that user and synth', async () => {
				assert.equal((await exchangeState.getLengthOfEntries(account1, oBTC)).toString(), '1');
			});
			it('and the length is 0 for other conditions', async () => {
				assert.equal((await exchangeState.getLengthOfEntries(account1, oUSD)).toString(), '0');
				assert.equal((await exchangeState.getLengthOfEntries(account2, oBTC)).toString(), '0');
			});
			describe('when the entry is fetch by index 0', () => {
				let result;
				beforeEach(async () => {
					result = await exchangeState.getEntryAt(account1, oBTC, '0');
				});
			...
			describe('when another entry is added for the same user and synth', () => {
				...
				it('then the length is 2 for that user and synth', async () => {
					assert.equal((await exchangeState.getLengthOfEntries(account1, oBTC)).toString(), '2');
				});
				...
				describe('when the entry is fetch by index 1', () => {
					let result;
					beforeEach(async () => {
						result = await exchangeState.getEntryAt(account1, oBTC, '1');
					});
				...
				describe('when all entries are removed for that user and synth', () => {
					beforeEach(async () => {
						await exchangeState.removeEntries(account1, oBTC, {
							from: simulatedAssociatedContract,
						});
					});
					it('then the length is 0 for that user and synth', async () => {
						assert.equal((await exchangeState.getLengthOfEntries(account1, oBTC)).toString(), '0');
					});
				});
			});
		});
	...
	describe('getMaxTimestamp()', () => {
		...
		describe('when there is a single entry with timestamp 100', () => {
			...
			describe('when there is another entry with a different src and with timestamp 101', () => {
				beforeEach(async () => {
					await addExchangeEntry({ user: account1, src: oBTC, dest: sAUD, timestamp: '101' });
				});
				...
				describe('when there is a another entry with timestamp 50', () => {
					...
					describe('when there are unrelated entries at higher timestamps than 101', () => {
						beforeEach(async () => {
							await addExchangeEntry({ user: account1, dest: oBTC, timestamp: '500' });
